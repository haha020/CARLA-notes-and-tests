...
import pygame
import random
from CarlaGymEnv.misc import *
from CarlaGymEnv.route_planner import RoutePlanner
from CarlaGymEnv.render import BirdeyeRender
from skimage.transform import resize

class CarlaGymEnvTest(gym.Env):
    def __init__(self, params):
        ...
        if self.discrete:
            self.action_space = spaces.Discrete(
                self.n_acc*self.n_steer)  # 离散化连续空间，gym的函数
        else:
            self.action_space = spaces.Box(np.array([params['continuous_accel_range'][0],  # box 连续空间
        ...
        self.blueprint_library_cache = self.world.get_blueprint_library()
        ...
        self.collision_bp = self.blueprint_library_cache.find(
    ...
    def _try_spawn_random_walker_at(self, transform):
        ...
        walker_bp = random.choice(
            self.blueprint_library_cache.filter('walker.*'))
    ...
    def _get_actor_polygons(self, filt):
        ...
        actor_poly_dict = {}
        actor_list = self.world.get_actors()
        for actor in actor_list.filter(filt):
    ...
    def _try_spawn_ego_vehicle_at(self, transform):
        ...
        overlap = False
        for idx, poly in self.vehicle_polygons[-1].items(): # key: value, use value no key
    ...
    def _get_obs(self):
        ...
        actor_vehicle_list = self.world.get_actors().filter('vehicle.*')
        for actor in actor_vehicle_list:
